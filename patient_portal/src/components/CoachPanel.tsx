...
import { Timer, RotateCcw, Repeat, Clock, CheckCircle2, XCircle } from 'lucide-react';
...
export default function CoachPanel({ exercise }: CoachPanelProps) {
    const videoRef = useRef<HTMLVideoElement>(null);
    const [isPlaying, setIsPlaying] = useState(true);

    const { endRep } = useSessionTracker();
    const { initMediaPipe } = useMediaPipe(videoRef, endRep);
        ...
        <div className="coach-panel" style={{
            ...
            <div style={{ position: 'relative', background: '#000', aspectRatio: '16/9' }}>
                {isYouTube ? (
                    ...
                    <video ref={videoRef} src={exercise.videoUrl} loop muted autoPlay playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                )}
            </div>
            ...
            <div style={{
                ...
                {[
                    { icon: <Clock size={16} />, label: 'Goal', value: `${exercise.durationSeconds}s` },
                    { icon: <Repeat size={16} />, label: 'Reps', value: lastResult?.rep_count ?? 0 },
            ...
            {lastResult && lastResult.checks && (
                <div style={{
                    ...
                    {Object.entries(lastResult.checks).map(([name, [passed, value]]) => (
                        <div key={name} style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: 8,
