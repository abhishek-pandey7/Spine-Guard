...
export default function Dashboard() {
  ...
  }, [exerciseIndex, phases, selectedPhase]);
  ...
  const handleExerciseComplete = useCallback((repCount: number, elapsedSeconds: number) => {
    ...
    if (nextIndex < allPhaseExercises.length) {
      // Start rest timer before next exercise (Default 20s as requested)
      const restTime = 20; 
      ...
      {screen === 'rest' && selectedExercise && (
        <div className="rest-screen fade-in">
          <div className="rest-card">
            ...
            <h2>{exerciseResults[exerciseResults.length - 1]?.exerciseName} Done</h2>
            <p className="rest-sub">
              {exerciseResults[exerciseResults.length - 1]?.elapsedSeconds ?? 0}s of quality movement
            </p>
            <div className="rest-timer">
              {restSecondsLeft}
            </div>
            <p className="rest-next">
              Up next &nbsp;<strong>{currentPhaseExercises[exerciseIndex + 1]?.name}</strong>
            </p>
            <button
              onClick={skipRest}
              className="btn-secondary"
              style={{ marginTop: '24px', borderRadius: '20px', fontSize: '13px', padding: '8px 22px' }}
            >
              Skip Rest →
            </button>
          </div>
