...
// - In local dev: uses same host:port as page → Vite proxy forwards /ws/* to :8000
// - Over network: set VITE_WS_URL=ws://<server-ip>:8000 in a .env file
// - HTTPS: automatically uses wss:// protocol
function getWsBase(): string {
  if (import.meta.env.VITE_WS_URL) {
    return import.meta.env.VITE_WS_URL as string;
  }
  const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
  // window.location.host includes the port (e.g. "localhost:5174")
  // Vite proxy will intercept /ws/* and forward to backend:8000
